...
router.post("/queryById", async (req, res) => {
	...
	const findStr = {};
	let res1 = await db.find("about_history", findStr).catch((err) => res.status(200).send({ msg: err.message, code: 500 }));
	if (addViews) {
		let res2 = await db.addViews("about_history", findStr).catch((err) => res.status(200).send({ msg: err.message, code: 500 }));
	}

...
router.post("/add", async (req, res) => {
	...
	db.insertOne("service_computer", insertStr).then((success) => {
...
router.post("/query", async (req, res) => {
	...
	await Promise.all([db.findByPage("service_computer", whereStr, limitStr, sortStr, pageNo, pageSize), db.getTotal("service_computer")]).then((array) => {
		res.status(200).send({
			msg: "查询成功",
			code: 200,
...
router.post("/changeIsTop", (req, res) => {
	...
	db.updateOne("service_computer", whereStr, updateStr).then((success) => {
		res.status(200).send({
...
router.post("/del", async (req, res) => {
    ...
    const publisherDepartmentId = (await utils.getUserData(publisher)).departmentId;
    const publisherDepartmentName = (await db.find("system_department", {id: publisherDepartmentId}))[0].name;
    const notHasDelAuth = await utils.notHasDelAuth(token, id, "service_computer");

    if (notHasDelAuth) {
    	res.status(200).send({ msg: "删除失败，没有权限", code: 500, result: {} });
    	return;
    } 
	db.deleteOne("service_computer", delStr).then(async (success) => {
		try {
...
router.post("/queryById", async (req, res) => {
	...
	const findStr = { "id": id };
	let res1 = await db.find("service_computer", findStr).catch((err) => {
		res.status(200).send({ msg: err, code: 500});
	});
	if (addViews) {
		let res2 = await db.addViews("service_computer", findStr).catch((err) => {
			res.status(200).send({ msg: err, code: 500});
		});
	}
	res.status(200).send({
...
router.post("/queryList", async (req, res) => {
	...
	const sortStr = { "isTop": -1, "createtime": -1, "topTime": -1 };
	await Promise.all([db.findByPage("service_computer", {}, {"content": 0}, sortStr, pageNo, pageSize), db.getTotal("service_computer")]).then((array) => {
		res.status(200).send({
			msg: "查询成功",
			code: 200,
			data: { list: array[0], total: array[1] }

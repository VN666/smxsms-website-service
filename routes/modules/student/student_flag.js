...
router.post("/add", async (req, res) => {
	...
	db.insertOne("student_flag", insertStr).then((success) => {
...
router.post("/query", async (req, res) => {
	...
	await Promise.all([db.findByPage("student_flag", whereStr, limitStr, sortStr, pageNo, pageSize), db.getTotal("student_flag")]).then((array) => {
		res.status(200).send({
			msg: "查询成功",
			code: 200,
...
router.post("/changeIsTop", (req, res) => {
	...
	db.updateOne("student_flag", whereStr, updateStr).then((success) => {
		res.status(200).send({
...
router.post("/del", async (req, res) => {
	...
	const delStr = { "id": id };
	db.deleteOne("student_flag", delStr).then(async (success) => {
		try {
...
router.post("/queryById", async (req, res) => {
	...
	const findStr = { "id": id };
	let res1 = await db.find("student_flag", findStr).catch((err) => {
		res.status(200).send({ msg: err, code: 500});
	});
	if (addViews) {
		let res2 = await db.addViews("student_flag", findStr).catch((err) => {
			res.status(200).send({ msg: err, code: 500});
		});
	}
	res.status(200).send({
...
router.post("/queryList", async (req, res) => {
	...
	const sortStr = { "isTop": -1, "createtime": -1, "topTime": -1 };
	await Promise.all([db.findByPage("student_flag", {}, {"content": 0}, sortStr, pageNo, pageSize), db.getTotal("student_flag")]).then((array) => {
		res.status(200).send({
			msg: "查询成功",
			code: 200,
			data: { list: array[0], total: array[1] }
